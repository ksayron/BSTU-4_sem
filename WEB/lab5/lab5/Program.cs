        ...
        public static void Main(string[] args)
        {
            ...
            using (ICelebrityRepository repository = JSONCelebrityRepository.Create("Celebrities"))
            {
                ...
                .AddEndpointFilter(async (context,next) =>
                {
                    ...
                    if (celebrity == null) { throw new AddCelebrityException("/Celebrities error celevrity == null"); }
                    if (celebrity.Surname == null || celebrity.Surname.Length < 2) { throw new BadArgumentException("Post /Celebrities error Surname invalid"); }
                ...
                .AddEndpointFilter(async (context,next) =>
                {
                    Celebrity? celebrity = context.GetArgument<Celebrity>(0);
                    if (celebrity == null) { throw new AddCelebrityException("/Celebrities error celevrity == null"); }
                    if (repository.GetBySurname(celebrity.Surname).Length > 0) { throw new BadArgumentException("Post /Celebrities error Surname doubled"); }
                    return await next(context);
                })
                 .AddEndpointFilter(async (context, next) =>
                 {
                     ...
                     if (!File.Exists(Path.Combine(repository.JSON_Path, $"{fn}")))
                     {
