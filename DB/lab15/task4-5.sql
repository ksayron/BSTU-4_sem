...
as 
    ...
    begin 
        ...
        set @a3 = (select [Gender] from inserted);
        set @a4 = (select [Pulpit] from inserted);
        set @in = 'Новые значения: ' + @a1 + ',' + @a2 + ', ' + @a3 + ', ' + @a4;
        ...
        set @a3 = (select [Gender] from deleted);
        set @a4 = (select [Pulpit] from deleted);
        set @in = 'Старые значения: ' + @a1 + ',' + @a2 + ', ' + @a3 + ', ' + @a4 + ' | ' + @in;
...
update TEACHER set TEACHER = 'ПППП' where TEACHER = 'РМНВ'

--- task5
insert into TEACHER values ('test', 'test', 'м', 'TEST')
-----------------
